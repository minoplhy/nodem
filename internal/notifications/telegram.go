...
// SendNotification dispatches a markdown message to the Telegram chat.
func (t *TelegramNotifier) SendNotification(ctx context.Context, message, _ string) error {
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		...
		return fmt.Errorf("telegram sendMessage failed with status %d: %s", resp.StatusCode, string(bodyBytes))
	}

	return nil
}

// SendDnsUpdateNotification formats and sends batched DNS update alerts to Telegram.
func (t *TelegramNotifier) SendDnsUpdateNotification(ctx context.Context, group *db.TargetGroup, transitionsUp []TransitionUp, transitionsDown []string, ipsStatus []IPStatus) error {
	...
	return t.SendNotification(ctx, message, level)
}

// SendUnmanagedIpsNotification sends anomaly alerts to Telegram.
func (t *TelegramNotifier) SendUnmanagedIpsNotification(ctx context.Context, group *db.TargetGroup, newlyDetected, newlyCleared []string) error {
	var parts []string
