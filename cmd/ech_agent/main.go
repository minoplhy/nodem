...
func main() {
	...
	_ = godotenv.Load(".env.agent", ".env", "/etc/ech_agent/agent.env")

	if len(os.Args) > 1 {
		switch os.Args[1] {
		case "version", "-v", "--version":
			fmt.Printf("ech_agent (nodem-agent) %s\n", version.Full())
			return
		case "cluster":
...
func generateMasterIncludesConf(storageDir string) error {
	...
	sb.WriteString("# Automatically generated by ech_agent - multi-cluster configuration\n")
